...
-- LFLJ Courchevel — land ONLY on Rwy 22 (uphill). Rwy 04 landing is prohibited.
...
function check_disqualification()
    ...
    -- Wrong runway: touchdown in lower half means came in on Rwy 04 (prohibited at Courchevel)
    if along < (rwy_length / 2) then
        return true, "Wrong runway - land on Rwy 22 only"
    end
    return false, ""
end
...
inflight_popup = contextWindow {
  ...
  function show_flytolearn ()
    if ftl_logo_frame:isVisible () then
        ...
        sasl.setMenuItemName(flytolearn_menu, flytolearn_startitem, "Show FlyToLearn Competition")
    else
        ftl_logo_frame:setIsVisible (true)
        sasl.setMenuItemName(flytolearn_menu, flytolearn_startitem, "Hide FlyToLearn Competition")
    end
end

flytolearn_menuitem = appendMenuItem (PLUGINS_MENU_ID, "FlyToLearn Competition")
flytolearn_menu = createMenu ("", PLUGINS_MENU_ID, flytolearn_menuitem)
flytolearn_startitem = appendMenuItem (flytolearn_menu, "Show FlyToLearn Competition", show_flytolearn)
